    ...
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4" onClick={onClose}>
      <div className="w-full max-w-lg bg-white dark:bg-zinc-900 border border-gray-200 dark:border-zinc-800 rounded-2xl shadow-2xl flex flex-col max-h-[90vh]" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="flex items-center justify-between px-6 py-5 border-b border-gray-200 dark:border-zinc-800 shrink-0">
          <h2 className="text-gray-900 dark:text-white font-semibold text-lg">Editar proyecto</h2>
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-5 px-6 py-6 overflow-y-auto">
          ...
          {currentImages.length > 0 && (
            <div className="flex flex-col gap-2">
              <span className="text-sm font-medium text-gray-600 dark:text-zinc-300">Imágenes actuales</span>
              <div className="flex flex-wrap gap-2">
                {currentImages.map((url) => (
                  <div key={url} className="relative group/img w-20 h-20 rounded-xl overflow-hidden border border-gray-300 dark:border-zinc-700 shrink-0">
          ...
          {/* New images */}
          <div className="flex flex-col gap-2">
            <span className="text-sm font-medium text-gray-600 dark:text-zinc-300">Agregar imágenes <span className="text-gray-400 dark:text-zinc-500 font-normal">(opcional)</span></span>
