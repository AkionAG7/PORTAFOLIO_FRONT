...
interface Props {
  onClose: () => void
  onCreated: (dto: CreateLanguageDto) => Promise<void>
}
...
export default function CreateLanguageModal({ onClose, onCreated }: Props) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      setError('Error al crear el idioma. Intenta de nuevo.')
    } finally {
      setIsLoading(false)
    }
  }
    ...
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4" onClick={onClose}>
      <div className="w-full max-w-md bg-white dark:bg-zinc-900 border border-gray-200 dark:border-zinc-800 rounded-2xl shadow-2xl flex flex-col" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between px-6 py-5 border-b border-gray-200 dark:border-zinc-800">
          <h2 className="text-gray-900 dark:text-white font-semibold text-lg">Nuevo idioma</h2>
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-5 px-6 py-6">
          <div className="flex flex-col gap-2">
            <label htmlFor="lang-name" className="text-sm font-medium text-gray-600 dark:text-zinc-300">Nombre</label>
            <input
              id="lang-name"
              type="text"
              required
