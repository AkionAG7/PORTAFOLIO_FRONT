...
export default function UserDetailModal({ user, onClose, onToggleStatus, onRoleChange, isToggling, isChangingRole }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-zinc-900 border border-zinc-800 rounded-2xl w-full max-w-md p-6 flex flex-col gap-5 shadow-2xl">
        ...
        {/* Info rows */}
        <div className="flex flex-col divide-y divide-zinc-800 bg-zinc-800/30 rounded-xl overflow-hidden">
          {rows.map(({ label, value }) => (
            <div key={label} className="flex items-center justify-between px-4 py-3 gap-4">
              <span className="text-zinc-500 text-xs uppercase tracking-wide font-medium shrink-0">{label}</span>
        ...
        <div className="flex flex-col gap-2">
          <span className="text-zinc-500 text-xs uppercase tracking-wide font-medium">Cambiar rol</span>
