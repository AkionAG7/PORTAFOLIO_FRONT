...
import { useAllUserStacks } from '../hooks/useAllUserStacks'
...
export default function AllUserStacksTab() {
  ...
  const { entries, isLoading, toggleStatus } = useAllUserStacks()
        ...
        <div className="flex flex-col gap-6">
          {filtered.map(({ user: u, stacks }) => (
            <div key={u.id} className="flex flex-col gap-3">
              ...
              {stacks.length === 0 ? (
                <p className="text-gray-400 dark:text-zinc-600 text-sm pl-11">Sin tecnologías asignadas.</p>
              ) : (
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3 pl-11">
                  {stacks.map((s) => (
                    <StackCard
                      key={s.stack_id}
                      item={s}
                      onToggleStatus={isAdmin ? () => toggleStatus.mutate({ userId: u.id, stackId: s.stack_id }) : undefined}
                    />
                  ))}
                </div>
              )}
